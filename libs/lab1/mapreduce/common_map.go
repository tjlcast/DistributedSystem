...
func doMap(
	...
	// create intermediate_files(cacheFiles).
	...
	for i := range cacheFiles {
		...
		cacheFiles[i] 			= file
		cacheFilesEncoder[i] 	= json.NewEncoder(file)
	}

	for _, kv := range kvs {
		eleKey := kv.Key
		idx := getCacheIndex(eleKey, nReduce)

		err := cacheFilesEncoder[idx].Encode(&kv)
		if err != nil {
...
func getCacheIndex(key string, nReduce int) uint32 {
